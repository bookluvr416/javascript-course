...
class RecipeView extends View {
  ...
  _errMessage = 'No recipe found for this recipe. Please try again!';
  _successMessage = '';

  addHandlerRenderer(fn) {
    ['hashchange', 'load'].forEach((e) => window.addEventListener(e, fn));
  };
  ...
  addHandlerUpdateServings(fn) {
    this._parentElement.addEventListener('click', (e) => {
      ...
      if (!btn) return;
      if (btn.dataset.servings > 0) fn(parseInt(btn.dataset.servings));
    });
  }
    ...
    return `
      ...
      <div class="recipe__details">
        ...
        <div class="recipe__user-generated">
          ${this._data.key ? `<svg><use href="${icons}#icon-user"></use></svg>` : ''}
        </div>
